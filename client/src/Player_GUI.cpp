...
void on_hide_left(GtkWidget *widget, gpointer data)
{
    Player* player = (Player*) data;

    player->clearLeftList();
}
...
void Player::init(int argc, char* argv[]) {
    ...
    room_hbox = gtk_hbox_new(FALSE, 0);
    ...
    gtk_box_pack_end(GTK_BOX(room_hbox), player_widget, TRUE, TRUE, 0);
    ...
    show_rooms_button = gtk_button_new_with_label("Комнаты");
    g_signal_connect(show_rooms_button, "clicked", G_CALLBACK(on_show_rooms), this);

    hide_left_button = gtk_button_new_with_label("Скрыть");
    g_signal_connect(hide_left_button, "clicked", G_CALLBACK(on_hide_left), this);
    ...
    gtk_box_pack_start(GTK_BOX(left_box), hide_left_button, FALSE, FALSE, 0);
